...
- **文档入库与版本管理**：支持导入 PDF、DOCX、PPTX、XLSX、HTML、CSV、EPUB 等多种格式，自动计算文件哈希避免重复导入，支持同一文档下新增版本。
...
|------|------|
| 桌面界面 | PySide6 |
| 文档解析 | markitdown[all]、markitdown-ocr |
...
所有格式的文档统一由 [markitdown](https://github.com/microsoft/markitdown) 解析为 Markdown，再转换为内部 DocumentIR 结构。

支持格式：`.pdf`、`.docx`、`.pptx`、`.xlsx`、`.xls`、`.html`、`.htm`、`.csv`、`.json`、`.xml`、`.epub`、`.txt`

**OCR 支持**：在设置页配置 OpenAI 兼容 API 后，markitdown-ocr 插件自动启用，对扫描件 PDF 和图片内容进行 LLM 辅助识别。未配置 API Key 时静默跳过，不影响普通文档解析。
...
app/
  ...
  core/
    ...
    parser/   文档解析路由（markitdown_adapter + router）
