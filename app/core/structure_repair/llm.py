...
- 重复性、边界接近性（顶部/底部）、通用描述性文本（如“页码”、“文档名称”、“文档编号”、“公司名”）是页眉页脚噪声的强烈证据，无论是否包含数字或标识符。除非属于连续语义的一部分，否则应移除。
- 当内容与页眉页脚证据严格平衡时，应倾向保留。但如果文本项明显为重复位置元数据（如页码、文档标题、版权声明或位于精确顶/底端的日期），证据不模糊，必须移除。
